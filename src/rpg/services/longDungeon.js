  ...
  function awardCompletion(session) {
    ...
    for (const recipientId of recipients) {
      ...
      if (stats) {
        db.prepare(`
          UPDATE rpg_users
          SET level = ?, xp = ?, hp = ?, max_hp = ?, atk = ?, def = ?, magic_atk = ?,
              crit_rate = ?, crit_multi = ?, updated_at = ?
          WHERE telegram_user_id = ?
        `).run(
            level, xp, calculateLevelUpHp(user.hp, user.max_hp, stats.max_hp, level - user.level), stats.max_hp, stats.atk, stats.def, stats.magic_atk,
