...
test('weekly party raid requires a duo and rewards can only be claimed once', () => {
  ...
  assert.equal(claim.reward.gold > 10, true);
  assert.equal(db.prepare("SELECT gold FROM rpg_users WHERE telegram_user_id='1'").get().gold, 49990 + claim.reward.gold);
...
test('duo bounty combines member actions and rewards each contributor once', () => {
  ...
  const highBalanceClaim = coop.claim('1');
  assert.equal(highBalanceClaim.success, true);
  assert.equal(highBalanceClaim.reward.gold > 0, true);
  assert.equal(db.prepare("SELECT gold FROM rpg_users WHERE telegram_user_id='1'").get().gold, 50000 + highBalanceClaim.reward.gold);
...
test('duo long dungeon shares checkpoints and grants idempotent rewards to both players', () => {
  const db = createTestDb();
  publishDungeons(db, loadDungeons());
  const social = createSocialService(db, { now: () => 2_000_000_000 });
  social.createParty('1');
