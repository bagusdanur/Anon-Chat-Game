import{c as e,l as t,u as n}from"./index-B39UAyRg.js";async function r(t){t.innerHTML=`<div class="page-enter"><div class="skeleton" style="height:400px"></div></div>`;try{let[n,r,i,a,o]=await Promise.all([e.get(`/api/bosses`),e.get(`/api/monsters`),e.get(`/api/shops`),e.get(`/api/crafting`),e.get(`/api/quests`)]),s={bosses:{title:`Bosses`,data:n,endpoint:`/api/bosses`},monsters:{title:`Monsters`,data:r,endpoint:`/api/monsters`},shops:{title:`Shops`,data:i,endpoint:`/api/shops`},crafting:{title:`Crafting`,data:a,endpoint:`/api/crafting`},quests:{title:`Quests`,data:o,endpoint:`/api/quests`}},c=`bosses`,l=`form`,u=()=>{let e=`<div class="tabs" style="display:flex;gap:10px;margin-bottom:15px;border-bottom:2px solid var(--border);padding-bottom:10px">`;for(let t of Object.keys(s))e+=`<button class="btn btn-${t===c?`primary`:`secondary`} tab-btn" data-tab="${t}">${s[t].title}</button>`;return e+=`</div>`,e},d=()=>{let e=s[c],t=`
