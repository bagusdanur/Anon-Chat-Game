import{c as e,l as t,u as n}from"./index-CMn0ttVB.js";async function r(t){t.innerHTML=`<div class="page-enter"><div class="skeleton" style="height:300px"></div></div>`;try{a=await e.get(`/api/classes`),t.innerHTML=`
    ...
    <div class="card" style="margin-bottom:20px;position:relative">
      <button class="btn btn-danger" style="position:absolute;top:15px;right:15px;padding:6px 12px;min-width:0;z-index:10" onclick="window.deleteClass(${t})">X</button>
      
      <div class="card-body" style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
        <div class="form-group" style="margin:0">
          <label class="form-label">ID (Unik)</label>
          <input type="text" class="form-control" value="${e.id}" onchange="window.updateClass(${t}, 'id', this.value)">
        </div>
        <div class="form-group" style="margin:0">
          <label class="form-label">Display Name (Emoji + Nama)</label>
          <input type="text" class="form-control" value="${e.name}" onchange="window.updateClass(${t}, 'name', this.value)">
        </div>
        <div class="form-group" style="margin:0">
          <label class="form-label">Base HP / ATK / DEF / Magic</label>
          <div style="display:flex;gap:10px">
            <input type="number" class="form-control" placeholder="HP" value="${e.base_hp}" onchange="window.updateClass(${t}, 'base_hp', this.value)">
            <input type="number" class="form-control" placeholder="ATK" value="${e.base_atk}" onchange="window.updateClass(${t}, 'base_atk', this.value)">
            <input type="number" class="form-control" placeholder="DEF" value="${e.base_def}" onchange="window.updateClass(${t}, 'base_def', this.value)">
            <input type="number" class="form-control" placeholder="MAG" value="${e.base_magic_atk}" onchange="window.updateClass(${t}, 'base_magic_atk', this.value)">
          </div>
        </div>
        <div class="form-group" style="margin:0">
          <label class="form-label">Growth (HP / ATK / DEF / Magic) per level</label>
          <div style="display:flex;gap:10px">
            <input type="number" class="form-control" placeholder="HP+" step="0.1" value="${e.growth.hp}" onchange="window.updateClass(${t}, 'growth.hp', this.value)">
            <input type="number" class="form-control" placeholder="ATK+" step="0.1" value="${e.growth.atk}" onchange="window.updateClass(${t}, 'growth.atk', this.value)">
            <input type="number" class="form-control" placeholder="DEF+" step="0.1" value="${e.growth.def}" onchange="window.updateClass(${t}, 'growth.def', this.value)">
            <input type="number" class="form-control" placeholder="MAG+" step="0.1" value="${e.growth.magic_atk}" onchange="window.updateClass(${t}, 'growth.magic_atk', this.value)">
          </div>
        </div>
        <div class="form-group" style="grid-column:1 / -1;border-top:2px solid var(--border-dark);padding-top:15px;margin:0">
          ...
          <div style="display:flex;gap:15px">
            <input type="text" class="form-control" style="flex:1" placeholder="Skill Name" value="${e.skillName}" onchange="window.updateClass(${t}, 'skillName', this.value)">
            <input type="text" class="form-control" style="flex:2" placeholder="Description" value="${e.skillDesc}" onchange="window.updateClass(${t}, 'skillDesc', this.value)">
            <input type="number" class="form-control" style="width:100px" step="0.1" placeholder="Mult" value="${e.skillMulti}" onchange="window.updateClass(${t}, 'skillMulti', this.value)" title="Skill Multiplier">
          </div>
        </div>
      </div>
    </div>
