...
const consumedInteractions = new Map();
...
function consumeInteraction(id, userId) {
  const key = `${id}:${userId}`;
  const now = Date.now();
  for (const [stored, expires] of consumedInteractions) if (expires <= now) consumedInteractions.delete(stored);
  if (consumedInteractions.has(key)) return false;
  consumedInteractions.set(key, now + 15 * 60 * 1000);
  return true;
}
