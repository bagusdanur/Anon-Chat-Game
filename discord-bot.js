...
const { xpToNextLevel, calcStats, getInventory, getOrCreateUser, createUser, spendGold, addGold, addItem, getCatalogItem } = require('./src/rpg/db_rpg');
...
function shopPage(userId, page = 1) {
  const user = getOrCreateUser(userId);
  const items = SHOP_ITEMS.filter(item => (user?.level || 1) >= (item.min_level || 1));
  const totalPages = Math.max(1, Math.ceil(items.length / DISCORD_PAGE_SIZE));
  const safePage = Math.min(totalPages, Math.max(1, page));
  const rows = items.slice((safePage - 1) * DISCORD_PAGE_SIZE, safePage * DISCORD_PAGE_SIZE);
  const description = rows.length ? rows.map((entry, index) => {
    const item = getCatalogItem(entry.item_id);
    const number = (safePage - 1) * DISCORD_PAGE_SIZE + index + 1;
    return `${number}. ${item?.display_name || entry.item_id} — ${entry.buy_price}g`;
  }).join('\n') : 'Shop kosong.';
  const controls = rows.map((entry, index) => new ButtonBuilder().setCustomId(`discord:shop:buy:${(safePage - 1) * DISCORD_PAGE_SIZE + index + 1}`).setLabel(`Beli ${(safePage - 1) * DISCORD_PAGE_SIZE + index + 1}`).setStyle(ButtonStyle.Success));
  const buttonRows = [];
...
function discordPayloads(message, options={}, privateReply=false) {
  const chunks=splitDiscordText(message);
  return chunks.map((content,index)=>({embeds:[new EmbedBuilder().setColor(0x7c3aed).setDescription(content)],components:index===chunks.length-1?buttons(options):[],...(privateReply?{flags:MessageFlags.Ephemeral}: {})}));
}
...
client.on('interactionCreate',async interaction=>{try{if (interaction.isAutocomplete()) { const q=(interaction.options.getString('input')||'').toLowerCase(); const key=ensureDiscordIdentity(interaction.user.id,interaction.guildId); let v=[]; if(['shop','buy'].includes(interaction.commandName)) v=SHOP_ITEMS.map(x=>({name:String(x.id)+'. '+String(x.item_id),value:String(x.item_id)})); else if(['inv','use','equip','sell'].includes(interaction.commandName)) v=getInventory(key).map((x,i)=>({name:String(i+1)+'. '+x.display_name+' x'+x.quantity,value:String(i+1)})); return interaction.respond(v.filter(x=>x.name.toLowerCase().includes(q)||x.value.toLowerCase().includes(q)).slice(0,25)); } if(interaction.isButton() && interaction.customId.startsWith('discord:shop:buy:')) { const number=Number(interaction.customId.split(':').pop()); const userKey=ensureDiscordIdentity(interaction.user.id,interaction.guildId); const user=getOrCreateUser(userKey); const item=SHOP_ITEMS.filter(x=>(user?.level||1)>=(x.min_level||1))[number-1]; if(!item)return interaction.reply({content:'Item shop tidak valid.',flags:MessageFlags.Ephemeral}); const catalog=getCatalogItem(item.item_id); return interaction.reply({content:'Konfirmasi pembelian '+(catalog?.display_name||item.item_id)+' seharga '+item.buy_price+'g?',flags:MessageFlags.Ephemeral,components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('discord:shop:confirm:'+number).setLabel('Konfirmasi beli').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId('discord:shop:cancel').setLabel('Batal').setStyle(ButtonStyle.Danger))]}); } if(interaction.isButton() && interaction.customId.startsWith('discord:shop:confirm:')) { const number=Number(interaction.customId.split(':').pop()); const userKey=ensureDiscordIdentity(interaction.user.id,interaction.guildId); const user=getOrCreateUser(userKey); const item=SHOP_ITEMS.filter(x=>(user?.level||1)>=(x.min_level||1))[number-1]; if(!item)return interaction.update({content:'Item shop tidak valid.',components:[]}); if(!spendGold(userKey,item.buy_price))return interaction.update({content:'Gold tidak cukup.',components:[]}); if(!addItem(userKey,item.item_id)){ addGold(userKey,item.buy_price); return interaction.update({content:'Inventory penuh.',components:[]}); } return interaction.update({content:'Pembelian berhasil: '+item.item_id,components:[]}); } if(interaction.isButton() && interaction.customId === 'discord:profile:inv') { const view=inventoryPage(ensureDiscordIdentity(interaction.user.id,interaction.guildId),1); return interaction.update({embeds:[new EmbedBuilder().setColor(0x7c3aed).setDescription(view.text)],components:view.components}); } if(interaction.isButton() && interaction.customId === 'discord:profile:shop') { const view=shopPage(ensureDiscordIdentity(interaction.user.id,interaction.guildId),1); return interaction.update({embeds:[new EmbedBuilder().setColor(0x7c3aed).setDescription(view.text)],components:view.components}); } if(interaction.isButton() && interaction.customId === 'discord:profile:skill') return interaction.update({content:'Buka /skill untuk melihat dan mengelola skill karakter.',embeds:[],components:[]}); if(interaction.isButton() && interaction.customId.startsWith('discord:shop:page:')) { const view=shopPage(ensureDiscordIdentity(interaction.user.id,interaction.guildId),Number(interaction.customId.split(':').pop())); return interaction.update({embeds:[new EmbedBuilder().setColor(0x7c3aed).setDescription(view.text)],components:view.components}); } if(interaction.isButton() && interaction.customId === 'discord:shop:cancel') return interaction.update({content:'Pembelian dibatalkan.',components:[]}); if(interaction.isButton() && interaction.customId.startsWith('discord:inv:page:')) { const view=inventoryPage(ensureDiscordIdentity(interaction.user.id,interaction.guildId),Number(interaction.customId.split(':').pop())); return interaction.update({embeds:[new EmbedBuilder().setColor(0x7c3aed).setDescription(view.text)],components:view.components}); } if(interaction.isButton() && (interaction.customId.startsWith('discord_party_accept:') || interaction.customId.startsWith('discord_party_decline:'))){ await interaction.deferUpdate(); const inviteOwner=interaction.customId.split(':')[1]; if(inviteOwner && inviteOwner!==String(interaction.user.id)) return interaction.editReply({content:'❌ Undangan ini bukan untuk akunmu.',components:[]}); const key=ensureDiscordIdentity(interaction.user.id,interaction.guildId); if(interaction.customId.startsWith('discord_party_accept:')){ const result=social.acceptInvite(key); return interaction.editReply({content:result.success?'✅ Kamu bergabung ke party #'+result.partyId:'❌ '+result.reason,components:[]}); } const result=social.leaveParty(key); return interaction.editReply({content:'❌ Undangan party ditolak.',components:[]}); } if(interaction.isButton()){let h=actions.get(interaction.customId); let match=null; if(!h){ for(const [pattern, candidate] of actions){ if(pattern instanceof RegExp){ const found=interaction.customId.match(pattern); if(found){h=candidate;match=found;break;} } } } if(!h)return interaction.reply({content:'Aksi sudah kedaluwarsa.'}); const ctx=ctxFor(interaction); if(match) { ctx.match=match; ctx.callbackQuery={data:interaction.customId}; } await h(ctx); await ctx.flush(); return;}if(!interaction.isChatInputCommand())return;const input=interaction.commandName==='profile'?(interaction.options.getString('class')||''):(interaction.options.getString('input')||''); const nativeTrade=interaction.commandName==='trade' ? {action:interaction.options.getString('action')||'status',type:interaction.options.getString('type'),amount:interaction.options.getInteger('amount'),tradeId:interaction.options.getInteger('trade_id')} : null; const nativeMarket=interaction.commandName==='market' ? {action:interaction.options.getString('action')||'browse',item:interaction.options.getInteger('item'),quantity:interaction.options.getInteger('quantity'),price:interaction.options.getInteger('price')} : null; const nativeGuild=interaction.commandName==='guild' ? {action:interaction.options.getString('action')||'info',value:interaction.options.getString('value')} : null; const target=interaction.options.getUser('user'); const actorKey=ensureDiscordIdentity(interaction.user.id,interaction.guildId);
    const expectedChannel = interaction.guildId && !PRIVATE_COMMANDS.has(interaction.commandName) ? COMMAND_CHANNEL[interaction.commandName] : null;
