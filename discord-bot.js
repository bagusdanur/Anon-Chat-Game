...
function ctxFor(interaction, text='') {
  ...
  const ctx = {
    ...
    reply: async (message, options={}) => { const payload={content:cleanText(message),components:buttons(options)}; if(!answered){answered=true;return interaction.reply(payload);} return interaction.followUp(payload); },
...
client.on('interactionCreate',async interaction=>{try{if(interaction.isButton()){const h=actions.get(interaction.customId);if(!h)return interaction.reply({content:'Aksi sudah kedaluwarsa.'});await h(ctxFor(interaction));return;}if(!interaction.isChatInputCommand())return;const input=interaction.commandName==='profile'?(interaction.options.getString('class')||''):(interaction.options.getString('input')||'');const h=handlers.get(interaction.commandName);if(!h)return interaction.reply({content:'Command belum tersedia.'});await h(ctxFor(interaction,'/'+interaction.commandName+(input?' '+input:'')),()=>{});if(!interaction.replied&&!interaction.deferred)await interaction.reply({content:'Selesai.'});}catch(e){console.error('[Discord] interaction failed:',e.message);if(!interaction.replied&&!interaction.deferred)await interaction.reply({content:'Terjadi kesalahan internal.'}).catch(()=>{});else await interaction.followUp({content:'Terjadi kesalahan internal.'}).catch(()=>{});}});
