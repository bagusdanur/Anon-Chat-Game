...
function discordPayloads(message, options={}, privateReply=false, command='profile') {
  ...
  return chunks.map((content,index)=>{
    ...
    return { embeds:[new EmbedBuilder().setColor(0x7c3aed).setDescription(content)], components: discordUi.dedupeRows(actionRows.length + navigation.length <= 5 ? [...actionRows, ...navigation] : actionRows), ...(privateReply?{flags:MessageFlags.Ephemeral}: {}) };
  });
