...
const { xpToNextLevel, calcStats } = require('./src/rpg/db_rpg');
...
function commandJson() {
  return COMMANDS.map(([name,description]) => {
    ...
    else { c.addStringOption(o=>o.setName('input').setDescription('Argumen command').setRequired(false)); if(['party','dungeon','duel','raid','bounty','trade','coopcampaign','worldboss'].includes(name)) c.addUserOption(o=>o.setName('user').setDescription('Target anggota Discord').setRequired(false)); }
    return c.toJSON();
  });
}
